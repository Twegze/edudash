...
const materialThemeStyles = `
  ...
  .bg-surface { background-color: var(--md-sys-color-surface); color: var(--md-sys-color-on-surface); transition: var(--transition-colors); }
  .bg-surface-variant { background-color: var(--md-sys-color-surface-variant); color: var(--md-sys-color-on-surface); transition: var(--transition-colors); }
  .bg-primary { background-color: var(--md-sys-color-primary); color: var(--md-sys-color-on-primary); transition: var(--transition-colors); }
  .bg-primary-container { background-color: var(--md-sys-color-primary-container); color: var(--md-sys-color-on-primary-container); transition: var(--transition-colors); }
  .bg-secondary-container { background-color: var(--md-sys-color-secondary-container); color: var(--md-sys-color-on-secondary-container); transition: var(--transition-colors); }
  ...
  .text-on-surface-variant { color: var(--md-sys-color-on-surface-variant); transition: var(--transition-colors); }
  .text-on-primary-container { color: var(--md-sys-color-on-primary-container); transition: var(--transition-colors); }
  .border-outline { border-color: var(--md-sys-color-outline); transition: var(--transition-colors); }
...
const ZenThemeModal = ({ isOpen, onClose, themeMode, setThemeMode, themeColors, setThemeColors, customApiKey, setCustomApiKey, selectedModel, setSelectedModel, availableModels, modelsLoading }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="zen-grid-bg w-full max-w-sm rounded-[40px] shadow-2xl flex flex-col items-center p-6 relative overflow-hidden border border-white/10 animate-in zoom-in-95 duration-300">
...
const FilePreviewCard = ({ file, onRemove, compact = false }) => {
  ...
  if (!compact && isImage) {
    return (
      <div className="relative group rounded-2xl overflow-hidden border border-outline/20 bg-surface">
  ...
  if (!compact && isPDF) {
    return (
      <div className="relative group rounded-2xl overflow-hidden border border-outline/20 bg-surface h-[500px] flex flex-col">
        <div className="p-3 bg-surface-variant flex justify-between items-center border-b border-outline/10">
  ...
  return (
    <div className="flex items-center justify-between p-4 rounded-2xl bg-surface border border-outline/20 shadow-sm relative group">
      <div className="flex items-center gap-3 overflow-hidden">
        <div className={`p-3 rounded-xl bg-surface-variant ${colorClass}`}>
          <Icon size={24} />
        </div>
...
const InstructionsWidget = () => {
  ...
  return (
    <div className="md:col-span-2 bg-primary-container rounded-3xl p-4 md:p-6 flex flex-col md:flex-row items-center justify-around shadow-sm min-h-[160px] gap-4">
      {modes.map((mode) => {
        ...
        return (
          <button 
            ...
            className={`flex-1 text-center p-4 md:p-6 w-full rounded-2xl transition-all duration-300 border border-transparent ${isActive ? 'bg-gradient-primary scale-[1.02] shadow-xl border-outline/20' : isDimmed ? 'opacity-40 hover:opacity-70' : 'hover:bg-white/20'}`}
          >
            <div className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-3 shadow-sm transition-colors ${isActive ? 'bg-[var(--md-sys-color-on-primary)] text-primary' : 'bg-surface text-primary'}`}>
              <Icon size={32} />
            </div>
            <span className={`font-bold block ${isActive ? 'text-white' : 'text-on-primary-container'}`}>{mode.title}</span>
            <span className={`text-sm mt-1 block ${isActive ? 'text-white/90' : 'text-on-primary-container opacity-80'}`}>{mode.desc}</span>
          </button>
        )
      })}
    </div>
...
const GroupMakerWidget = ({ classes }) => {
  ...
  return (
    <div className="bg-surface-variant rounded-3xl p-6 flex flex-col shadow-sm h-full max-h-[400px]">
      ...
      {isGenerated ? (
        <div className="flex-1 flex flex-col overflow-hidden">
           <div className="flex-1 overflow-y-auto pr-2 grid grid-cols-2 gap-2 mb-4">
              {groups.map((g, i) => (
                <div key={i} className="bg-surface p-3 rounded-xl border border-outline/10 text-sm">
                  <div className="font-bold text-gradient-primary mb-1 border-b border-outline/10 pb-1">Groupe {i+1}</div>
           ...
           <button onClick={() => setIsGenerated(false)} className="w-full py-3 bg-surface rounded-full text-on-surface hover:bg-secondary-container transition-colors font-medium border border-outline/10">
        ...
        <div className="flex-1 flex flex-col gap-3 overflow-hidden">
            <select 
              ...
              className="w-full bg-surface p-2 rounded-xl text-sm border border-outline/10 focus:ring-2 focus:ring-primary text-on-surface outline-none shadow-inner"
            >
            ...
            <textarea
              ...
              className="flex-1 w-full bg-surface rounded-2xl p-4 resize-none border border-outline/10 focus:ring-2 focus:ring-primary text-sm text-on-surface outline-none shadow-inner"
              placeholder="Liste des élèves (un par ligne)..."
            />
            <div className="flex items-center gap-2 bg-surface p-2 rounded-2xl shadow-inner border border-outline/10">
              <label className="text-sm text-on-surface-variant ml-2 flex-1">Nb. de groupes :</label>
              <input 
                type="number" min="2" max="15" 
                value={numGroups} onChange={(e) => setNumGroups(parseInt(e.target.value)||2)}
                className="w-16 text-center bg-transparent border-none focus:ring-0 text-on-surface font-bold outline-none"
              />
            </div>
            <button 
              onClick={generateGroups}
              disabled={names.trim().length === 0}
              className="w-full py-4 bg-gradient-primary rounded-full font-semibold hover:opacity-90 transition-all shadow-md active:scale-[0.98]"
            >
              Créer les groupes
            </button>
        </div>
...
const TimerWidget = () => {
  ...
  return (
    <div className="bg-surface-variant rounded-3xl p-6 flex flex-col items-center justify-center shadow-sm h-full">
      <div className="flex items-center gap-2 mb-4 w-full">
        <Clock className="text-primary" size={24} />
        <h3 className="font-semibold text-lg">Chronomètre</h3>
      </div>
      <div className="text-6xl font-bold font-mono tracking-tighter text-gradient-primary mb-6">
        {formatTime(time)}
      </div>
      <div className="flex gap-4 mb-4">
        <button onClick={toggle} className="w-16 h-16 rounded-full bg-gradient-primary flex items-center justify-center hover:opacity-90 shadow-md transition-transform active:scale-95">
          {isActive ? <Pause fill="white" size={24} /> : <Play fill="white" size={24} className="ml-1" />}
        </button>
        <button onClick={reset} className="w-16 h-16 rounded-full bg-secondary-container flex items-center justify-center hover:opacity-90 shadow-sm transition-transform active:scale-95">
          <RotateCcw size={24} className="text-on-secondary-container" />
        </button>
      </div>
      {!isActive && (
        <div className="flex items-center gap-2 mt-4 bg-surface p-2 rounded-2xl shadow-inner">
          <input 
            type="number" min="1" max="90" value={inputMinutes} 
            onChange={(e) => setInputMinutes(parseInt(e.target.value) || 1)}
            className="w-16 text-center bg-transparent border-none focus:ring-0 text-on-surface font-semibold outline-none"
          />
          <span className="text-sm text-on-surface-variant">min</span>
          <button onClick={reset} className="ml-2 text-primary p-2 hover:bg-primary-container rounded-xl transition-colors">
...
const RandomPickerWidget = ({ classes }) => {
  ...
  return (
    <div className="bg-surface-variant rounded-3xl p-6 flex flex-col shadow-sm h-full">
      <div className="flex items-center gap-2 mb-4">
        <Users className="text-primary" size={24} />
      ...
      <div className="flex-1 flex flex-col gap-4">
        {selected ? (
          <div className="flex-1 bg-primary-container rounded-2xl flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in duration-300 shadow-inner">
            <span className="text-sm text-on-primary-container opacity-80 mb-2">
              {isPicking ? "Sélection en cours..." : "L'élève choisi est :"}
            </span>
            <span className="text-4xl font-bold text-gradient-primary">{selected}</span>
            {!isPicking && (
              <button 
                onClick={() => setSelected(null)}
                className="mt-6 text-sm px-4 py-2 bg-surface rounded-full shadow-sm hover:bg-secondary-container transition-colors text-on-surface"
              >
                Retour à la liste
              </button>
            )}
          ...
          <>
            ...
            <textarea
              ...
              className="flex-1 w-full bg-surface rounded-2xl p-4 resize-none border border-outline/10 focus:border-transparent focus:ring-2 focus:ring-[var(--md-sys-color-primary)] text-sm text-on-surface shadow-inner outline-none"
        ...
        <button 
          ...
          className="w-full py-4 bg-gradient-primary rounded-full font-semibold flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 transition-all shadow-md active:scale-[0.98]"
        >
  ...
  return (
    <div className="bg-surface-variant rounded-3xl p-6 flex flex-col shadow-sm h-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <button 
          ...
          className={`p-3 rounded-full flex items-center justify-center transition-colors shadow-sm ${
            isListening ? 'bg-[var(--md-sys-color-error-container)] text-[var(--md-sys-color-on-error-container)]' : 'bg-primary-container text-on-primary-container hover:opacity-80'
          }`}
        >
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6">
        <div className="w-full h-8 bg-surface rounded-full overflow-hidden border border-outline/20 shadow-inner">
          <div className={`h-full transition-all duration-100 ease-out ${barColor}`} style={{ width: `${Math.max(5, volume)}%` }} />
        </div>
        <div className="text-center">
          <div className="text-3xl font-bold text-gradient-primary mb-1">{volume} %</div>
          <div className="text-sm font-medium text-on-surface-variant uppercase tracking-wider">{statusText}</div>
        </div>
      </div>
      {!isListening && (
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 h-full min-h-[600px]">
      ...
      <div className="w-full md:w-1/3 lg:w-1/4 flex flex-col gap-4">
        <div className="bg-surface-variant rounded-3xl p-4 shadow-sm">
              ...
              <button
                ...
                className={`flex items-center justify-between px-4 py-3 rounded-2xl transition-all ${
                  selectedClassId === c.id 
                    ? 'bg-gradient-primary shadow-md font-medium' 
            ...
            {isAddingClass ? (
              <form onSubmit={handleAddClass} className="flex items-center gap-2 mt-2">
                <input 
                  ...
                  className="flex-1 bg-surface p-2 rounded-xl text-sm border border-outline/20 focus:border-primary focus:ring-1 focus:ring-primary text-on-surface outline-none"
                />
                <button type="submit" className="p-2 bg-gradient-primary rounded-xl shadow-sm hover:opacity-90 transition-opacity">
                  <CheckCircle2 size={16} />
                </button>
                <button type="button" onClick={() => setIsAddingClass(false)} className="p-2 bg-surface text-on-surface-variant rounded-xl shadow-sm hover:bg-outline/10 transition-colors">
                  <X size={16} />
                </button>
              </form>
            ) : (
              <button 
                ...
                className="mt-2 flex items-center justify-center gap-2 px-4 py-3 rounded-2xl border-2 border-dashed border-outline/30 text-on-surface-variant hover:bg-surface hover:text-primary transition-colors"
              >
      ...
      <div className="flex-1 flex flex-col gap-4">
        {selectedClass ? (
          <>
            {/* Header & SubTabs */}
            <div className="bg-surface-variant rounded-3xl p-4 shadow-sm flex flex-col gap-4">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h2 className="font-bold text-xl text-on-surface">Classe : {selectedClass.name}</h2>
                </div>
                
                {/* Sub-navigation Tabs */}
                <div className="flex bg-surface p-1 rounded-2xl shadow-inner border border-outline/10">
                  <button 
                    onClick={() => setActiveSubTab('feed')}
                    className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeSubTab === 'feed' ? 'bg-primary text-white shadow-md' : 'text-on-surface-variant hover:text-primary'}`}
                  >
                    Documents & Cours
                  </button>
                  <button 
                    onClick={() => setActiveSubTab('students')}
                    className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${activeSubTab === 'students' ? 'bg-primary text-white shadow-md' : 'text-on-surface-variant hover:text-primary'}`}
                  >
                    Liste des Élèves
                  </button>
                </div>
            ...
            {activeSubTab === 'students' ? (
              <div className="bg-surface rounded-3xl p-6 shadow-sm border border-outline/10 flex-1 flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between">
                  <h3 className="font-bold text-lg text-primary flex items-center gap-2">
                    <Users size={20} /> Gestion des élèves
                  </h3>
                  <span className="text-sm font-medium bg-secondary-container text-on-secondary-container px-3 py-1 rounded-full shadow-inner">
                ...
                <textarea
                  ...
                  className="flex-1 w-full bg-surface-variant rounded-2xl p-4 resize-none border border-outline/20 focus:border-primary focus:ring-2 focus:ring-primary text-on-surface shadow-inner outline-none min-h-[300px]"
                  placeholder="Ex:\nJean Dupont\nMarie Curie\nAlbert Einstein..."
                />
                ...
                <div className="flex items-center justify-end gap-4 mt-2">
                  ...
                  <button 
                    ...
                    className="px-6 py-3 rounded-full font-bold bg-gradient-primary shadow-md hover:opacity-90 flex items-center gap-2 transition-all active:scale-95"
                  >
              ...
              <>
                <div className="flex justify-end gap-2">
                  <div className="flex items-center gap-3 bg-surface p-2 rounded-2xl border border-outline/10 shadow-inner">
                    <Calendar size={18} className="text-primary ml-2" />
                    <input 
                      type="date" 
                      value={filterDate} 
                      onChange={(e) => setFilterDate(e.target.value)}
                      className="bg-transparent border-none focus:ring-0 text-sm text-on-surface cursor-pointer outline-none"
                    />
                    {filterDate && (
                      <button onClick={() => setFilterDate("")} className="mr-2 text-on-surface-variant hover:text-[var(--md-sys-color-error)] transition-colors">
                        <X size={16} />
                      </button>
                    )}
                  </div>
                ...
                {isAddingCourse ? (
                  <form onSubmit={handleAddCourse} className="bg-primary-container rounded-3xl p-6 shadow-md animate-in slide-in-from-top-4 fade-in">
                    <h3 className="font-bold text-on-primary-container mb-4 flex items-center gap-2">
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex gap-3">
                        <input 
                          ...
                          className="flex-1 bg-surface p-3 rounded-2xl border border-outline/20 text-on-surface focus:ring-2 focus:ring-primary outline-none"
                        />
                        <input 
                          type="date" required
                          value={newCourse.date} onChange={(e) => setNewCourse({...newCourse, date: e.target.value})}
                          className="bg-surface p-3 rounded-2xl border border-outline/20 text-on-surface focus:ring-2 focus:ring-primary outline-none"
                        />
                      </div>
                      ...
                      <textarea 
                        ...
                        className="w-full bg-surface p-3 rounded-2xl border border-outline/20 text-on-surface focus:ring-2 focus:ring-primary outline-none resize-none"
                      />

                      <div className="border-2 border-dashed border-outline/30 rounded-2xl p-4 bg-surface text-center relative overflow-hidden transition-colors hover:bg-surface-variant hover:border-primary">
                        <input 
                          type="file" 
                          ref={fileInputRef}
                        ...
                        />
                        {!newCourse.file ? (
                          <div className="flex flex-col items-center justify-center pointer-events-none text-on-surface-variant py-4">
                      ...
                      <div className="flex justify-end gap-2 mt-2">
                        <button type="button" onClick={() => setIsAddingCourse(false)} className="px-4 py-2 rounded-full font-medium text-on-surface-variant hover:bg-surface transition-colors">
                          Annuler
                        </button>
                        <button type="submit" disabled={!newCourse.title.trim()} className="px-6 py-2 rounded-full font-bold bg-gradient-primary shadow-md hover:opacity-90 disabled:opacity-50 flex items-center gap-2 transition-all">
                  ...
                  <button 
                    ...
                    className="bg-surface rounded-3xl p-4 shadow-sm border border-outline/10 text-on-surface-variant hover:text-primary transition-all flex items-center justify-center gap-2 group font-medium relative overflow-hidden"
                  >
                    <div className="absolute inset-0 bg-gradient-primary opacity-0 group-hover:opacity-10 transition-opacity"></div>
                    <div className="w-8 h-8 rounded-full bg-primary-container text-primary flex items-center justify-center group-hover:bg-gradient-primary group-hover:text-white transition-colors relative z-10">
                      <Plus size={20} />
                    </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 flex flex-col gap-4 pb-10">
                  {filteredCourses.length === 0 ? (
                    <div className="bg-surface-variant rounded-3xl p-10 text-center flex flex-col items-center justify-center text-on-surface-variant opacity-70 border border-dashed border-outline/20 h-full">
                    ...
                    filteredCourses.map(course => (
                      <div key={course.id} className="bg-surface rounded-3xl p-6 shadow-sm border border-outline/10 group relative animate-in fade-in slide-in-from-bottom-4 flex flex-col gap-4">
                        <div className="flex justify-between items-start">
                          <h3 className="font-bold text-lg text-gradient-primary flex items-center gap-2">
                            {course.file ? <Paperclip size={18} className="opacity-50 text-on-surface" /> : <BookOpen size={18} className="opacity-50 text-on-surface" />}
                            {course.title}
                          </h3>
                          <span className="text-xs font-medium px-3 py-1 bg-secondary-container text-on-secondary-container rounded-full flex items-center gap-1 shadow-inner">
                            <Calendar size={12} /> {new Date(course.date).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long' })}
                          </span>
                        </div>

                        {course.content && (
                          <div className="text-on-surface bg-surface-variant p-4 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap border border-outline/5">
                            {course.content}
                          </div>
                        )}
                        ...
                        <button 
                          ...
                          className="absolute top-4 right-4 p-2 text-red-500 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl bg-surface"
          ...
          <div className="bg-surface-variant rounded-3xl p-10 text-center flex flex-col items-center justify-center text-on-surface-variant h-full shadow-sm">
            <Users size={64} className="mb-4 text-primary opacity-50" />
            <p className="text-xl font-bold text-on-surface mb-2">Aucune classe sélectionnée</p>
...
const AiStudio = ({ classes, courses, customApiKey, selectedModel }) => {
  ...
  const renderMarkdown = (text) => {
    ...
    lines.forEach((line, idx) => {
      if (line.startsWith('### ')) { html.push(<h3 key={idx} className="text-gradient-primary">{line.substring(4).replace(/\*\*/g, '')}</h3>); }
      ...
      else if (line.startsWith('---')) { html.push(<hr key={idx} className="my-6 border-outline/20" />); }
      else if (line.startsWith('**') && line.endsWith('**')) {
        html.push(<p key={idx} className="font-bold mt-4 text-on-surface">{line.replace(/\*\*/g, '')}</p>);
      }
      else if (line.trim() !== "") {
        html.push(<p key={idx} dangerouslySetInnerHTML={{__html: parseInline(line)}} className="mb-2 text-on-surface" />);
      }
    });
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full min-h-[600px]">
      <div className="col-span-1 bg-surface-variant rounded-3xl p-6 shadow-sm flex flex-col h-fit">
        <div className="flex items-center gap-2 mb-6">
          <Sparkles className="text-primary" size={24} />
          <h2 className="font-bold text-xl text-on-surface">Studio IA ✨</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-5">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              <button 
                ...
                className={`flex items-center gap-3 p-3 rounded-2xl border text-left transition-all ${selectedTool === "quiz" ? "border-primary bg-primary/10 text-primary shadow-sm" : "border-outline/20 bg-surface text-on-surface-variant hover:bg-surface/80"}`}
              >
                <div className={`p-2 rounded-full ${selectedTool === "quiz" ? "bg-primary text-white" : "bg-surface-variant"}`}>
                  <ListChecks size={18} />
                </div>
                <div>
              ...
              <button 
                ...
                className={`flex items-center gap-3 p-3 rounded-2xl border text-left transition-all ${selectedTool === "flashcards" ? "border-primary bg-primary/10 text-primary shadow-sm" : "border-outline/20 bg-surface text-on-surface-variant hover:bg-surface/80"}`}
              >
                <div className={`p-2 rounded-full ${selectedTool === "flashcards" ? "bg-primary text-white" : "bg-surface-variant"}`}>
                  <BookOpen size={18} />
                </div>
                <div>
              ...
              <button 
                ...
                className={`flex items-center gap-3 p-3 rounded-2xl border text-left transition-all ${selectedTool === "simplify" ? "border-primary bg-primary/10 text-primary shadow-sm" : "border-outline/20 bg-surface text-on-surface-variant hover:bg-surface/80"}`}
              >
                <div className={`p-2 rounded-full ${selectedTool === "simplify" ? "bg-primary text-white" : "bg-surface-variant"}`}>
                  <Lightbulb size={18} />
                </div>
                <div>
          ...
          <div className="space-y-2 mt-2">
            ...
            <div className="relative">
              <select 
                ...
                className="w-full bg-surface p-3 pr-10 rounded-2xl border border-outline/10 focus:ring-2 focus:ring-primary text-on-surface appearance-none outline-none shadow-inner"
              >
          ...
          {sourceType === "new" && (
            <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
              <textarea 
                ...
                className="w-full bg-surface p-3 rounded-2xl border border-outline/10 focus:ring-2 focus:ring-primary text-on-surface resize-none shadow-inner outline-none"
          ...
          {(selectedTool === "quiz" || selectedTool === "flashcards") && (
            <div className="space-y-2 mb-2 animate-in fade-in">
              <label className="text-sm font-medium text-on-surface-variant flex items-center justify-between">
                ...
                <span className="font-bold text-gradient-primary">{numItems} {selectedTool === "quiz" ? "questions" : "cartes"}</span>
              </label>
              <input 
          ...
          <button 
            ...
            className="mt-auto w-full py-4 bg-gradient-primary rounded-full font-bold flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-50 transition-all shadow-md active:scale-95"
          >
            {loading ? (
      ...
      <div className="col-span-1 lg:col-span-2 bg-surface rounded-3xl p-8 border border-outline/10 shadow-inner overflow-y-auto relative h-[600px] lg:h-auto">
        ...
        {result && !loading && (
          <div className="animate-in slide-in-from-bottom-4 duration-500 bg-surface">
...
export default function App() {
  ...
  return (
    <>
      ...
      <div className={`theme-wrapper ${isDark ? 'dark' : ''} min-h-screen bg-surface font-sans flex transition-colors duration-300`}>
        
        {/* Sidebar */}
        <aside className="w-20 md:w-64 bg-surface-variant border-r border-outline/10 flex flex-col p-4 z-20 transition-all duration-300 shadow-sm">
          <div className="flex items-center justify-center md:justify-start gap-3 p-2 mb-8 mt-4">
            <div className="w-10 h-10 min-w-[40px] rounded-xl bg-gradient-primary flex items-center justify-center shadow-lg">
              <GraduationCap size={24} className="text-white" />
            </div>
            <h1 className="font-bold text-xl hidden md:block text-gradient-primary tracking-tight">EduDash</h1>
          </div>
          ...
          <nav className="flex-1 flex flex-col gap-2">
            <button 
              ...
              className={`flex items-center justify-center md:justify-start gap-4 p-4 rounded-full transition-all duration-200 ${
                activeTab === 'classes' ? 'bg-primary-container text-on-primary-container font-bold shadow-sm' : 'text-on-surface-variant hover:bg-surface hover:text-on-surface'
              }`}
              title="Mes Classes"
            >
            ...
            <button 
              ...
              className={`flex items-center justify-center md:justify-start gap-4 p-4 rounded-full transition-all duration-200 ${
                activeTab === 'tools' ? 'bg-primary-container text-on-primary-container font-bold shadow-sm' : 'text-on-surface-variant hover:bg-surface hover:text-on-surface'
              }`}
              title="Outils"
            >
            ...
            <button 
              ...
              className={`flex items-center justify-center md:justify-start gap-4 p-4 rounded-full transition-all duration-200 ${
                activeTab === 'ai' ? 'bg-primary-container text-on-primary-container font-bold shadow-sm' : 'text-on-surface-variant hover:bg-surface hover:text-on-surface'
              }`}
              title="Studio IA"
            >
          ...
          <div className="mt-auto">
            <button 
              ...
              className="flex items-center justify-center md:justify-start gap-4 p-4 rounded-full w-full text-on-surface-variant hover:bg-surface-variant transition-colors"
            >
              <Settings size={24} />
        ...
        <main className="flex-1 p-4 md:p-8 overflow-y-auto">
          ...
          <div className="max-w-7xl mx-auto h-[calc(100%-100px)]">
            {activeTab === 'tools' && (
